...
#define AUDIO_MUSIC_FADE_VALUE 4
...
typedef struct privateAudioDevice
{
    ...
    SDL_AudioSpec audioSpec;  //I just renamed this while drunk from want to audioSpec 
...
void initAudio(void)
{
    ...
    SDL_memset(&(gDevice->audioSpec), 0, sizeof(gDevice->audioSpec));
    
    (gDevice->audioSpec).freq = AUDIO_FREQUENCY;
    (gDevice->audioSpec).format = AUDIO_FORMAT;
    (gDevice->audioSpec).channels = AUDIO_CHANNELS;
    (gDevice->audioSpec).samples = AUDIO_SAMPLES;
    (gDevice->audioSpec).callback = audioCallback;
    (gDevice->audioSpec).userdata = calloc(1, sizeof(Audio));
    
    global = (Audio *) (gDevice->audioSpec).userdata;
    ...
    if((gDevice->device = SDL_OpenAudioDevice(NULL, 0, &(gDevice->audioSpec), NULL, SDL_AUDIO_ALLOW_CHANGES)) == 0)
    {
...
void endAudio(void)
{
    if(gDevice->audioEnabled)
    {
        ...
        freeAudio((Audio *) (gDevice->audioSpec).userdata);
...
static inline void playAudio(const char * filename, Audio * audio, uint8_t loop, int volume)
{
    ...
    if(loop == 1)
    {
        addMusic((Audio *) (gDevice->audioSpec).userdata, newAudio);
    }
    else
    {
        addAudio((Audio *) (gDevice->audioSpec).userdata, newAudio);
    }
